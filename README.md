...
Este trabajo consistía en implementar un programa que, utilizando el PWM, modificase la intensidad de los LEDs de la placa en función de los valores obtenidos por uno de sus sensores. En mi caso, utilicé el módulo TSI (Touch Sensing Input)
...
Para calcular en qué zona del slider se está presionando, me ayudé de las funciones dadas en el foro Creé una versión propia de las funciones Touch_Scan_LH y Touch_Scan_HL que acepta el canal como parámetro con el objetivo de evitar duplicación de código. Además, por seguridad, añadí la siguiente línea:
...
Este es un detalle menos importante, pero lo consideré de interés igualmente. Creé una función para obtener los valores del LED ya que, en caso de que se quisiese extender el proyecto para usar cualquier otro tipo de sensor, la firma de la función podría mantenerse constante. Es decir, se podría hacer un struct de funciones con esa misma firma y que, dependiendo de alguna variable que indicase el "modo", se llamase a una de ellas.
